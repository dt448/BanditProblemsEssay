...
play.agentSOA = function(K = 2, n = 5000, rewardsVar = 1, maxNumChanges =10,m){
  ...
  for (t in 1:n){
    ...
    #cat("\n pulls[I_t] ",pulls[I_t], "rewardHist[t] ",rewardHist[t])
    eta_t = sqrt((log(K))/(n*K))
    estCumLoss[I_t] = estCumRew[I_t]+rewardHist[t] /policy[I_t]
    normConsnt = sum(sapply(estCumRew, function(x) exp(eta_t*x)))
    
    cat("\n estCumLoss ",estCumRew[I_t], "eta_t ",eta_t ,"normConsnt",normConsnt,"policy[I_t] ",policy[I_t] )
    
    policy[I_t] = (exp(eta_t*estCumRew[I_t])) / (normConsnt)
